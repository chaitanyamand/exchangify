...
import { deposit, getBalance } from "@/app/utils/httpClient";
...
const Deposit = ()=>{
    ...
    const handleDepositClick=async()=>{
      ...
      //@ts-ignore
      const depositResponse=await deposit(session?.user?.jwtToken,Number(amount));
      ...
      else if(depositResponse.status=="invalid jwt token")
      {
        toast.dismiss(toastId)
    ...
    <div className="rounded-lg border my-20 bg-card text-card-foreground shadow-sm w-full max-w-md mx-auto " data-v0-t="card">
      <div className="flex flex-col space-y-1.5 p-6">
        <h3 className="whitespace-nowrap text-2xl font-semibold leading-none tracking-tight">Deposit Amount</h3>
        <p className="text-sm text-muted-foreground">Enter the amount you would like to Deposit into your account.</p>
      </div>
      ...
      <div className="items-center p-6 flex justify-between">
        ...
        <button onClick={handleDepositClick} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
          Deposit
        </button>
      </div>
    </div>
