...
export function Depth({ market,depthTrade }: {market: string,depthTrade:any}) {
    ...
    useEffect(() => {
        ...
        SignalingManager.getInstance().registerCallback("ticker",(data: Partial<Ticker>)=>{
            setPrice((prevPrice)=>{
                return data.lastPrice || prevPrice
            })
        },`TICKERDEPTHPRICE-${market}`)
        ...
        return () => {
            ...
            SignalingManager.getInstance().deRegisterCallback("ticker", `TICKERDEPTHPRICE-${market}`);
        }
    }, [])
